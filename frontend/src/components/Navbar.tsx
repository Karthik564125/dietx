...
const Navbar = ({ setIsAuthenticated }: NavbarProps) => {
  ...
  return (
    <>
            ...
            <div className="hidden md:flex items-center gap-2">
              {[
                ...
                <button
                  ...
                  className={`px-5 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${location.pathname === link.path
              ...
              <button
                ...
                className={`flex items-center gap-3 p-1.5 pr-5 rounded-2xl border transition-all ${location.pathname === '/profile'
      ...
      <div
        className={`fixed inset-0 z-50 transition-opacity duration-700 md:hidden ${isOpen ? 'bg-slate-900/40 backdrop-blur-md opacity-100' : 'opacity-0 pointer-events-none'
          }`}
        onClick={() => setIsOpen(false)}
      />

      {/* Mobile Side Drawer */}
      <div className={`fixed inset-y-0 right-0 z-50 w-[85vw] max-w-sm bg-black/60 backdrop-blur-3xl border-l border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.4)] rounded-l-[3rem] transition-transform duration-700 md:hidden flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'
        }`}>
        ...
        <div className="flex-1 px-8 space-y-4 mt-10">
          {[
            ...
            <button
              key={link.path}
              onClick={() => navigate(link.path)}
              className={`flex items-center justify-between w-full p-5 rounded-[2rem] font-black transition-all duration-500 ${location.pathname === link.path
