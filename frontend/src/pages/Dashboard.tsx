...
import { Calendar, Salad, TrendingUp, Plus, ChevronRight, Droplet, Activity, Zap, ArrowUpRight, Save, Moon, Sparkles, Search, Trash2, Utensils, Coffee, Sun, MoonStar, Target, ChevronDown, Minus } from 'lucide-react';
          ...
          <div className="lg:col-span-2 space-y-10">
                ...
                <div className="p-8 sm:p-10 space-y-10">
                  <header className="flex justify-between items-center">
                    ...
                    <button 
                      ...
                      className={`px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center gap-2 shadow-lg ${
                        showAddFood 
                  ...
                  <div className="space-y-12">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
                        ...
                        <div className="lg:col-span-8 flex flex-col">
                            <div className="relative p-10 bg-emerald-50/30 rounded-[3.5rem] border border-emerald-100/50 overflow-hidden group/main h-full flex items-center">
                                ...
                                <div className="flex flex-col sm:flex-row items-center gap-12 relative z-10 w-full">
                                   <CircularProgress 
                                      size={160} 
                                      strokeWidth={14} 
                                      percentage={calPct} 
                                   ...
                                   <div className="flex-1 space-y-6 text-center sm:text-left">
                                      ...
                                      <div className="flex items-center gap-6 justify-center sm:justify-start">
                                         <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-bold text-slate-400">{calorieTarget} Target</span>
                                         </div>
                                         <div className="w-1 h-1 bg-slate-200 rounded-full"></div>
                                         <div className="flex items-center gap-2">
                                            <TrendingUp size={14} className="text-emerald-500" />
                                            <span className="text-xs font-bold text-emerald-600">{calPct.toFixed(0)}% Done</span>
                                         </div>
                                      </div>
                                   </div>
                                </div>
                                    ...
                                    <div className="space-y-5">
                                        {[
                                          ...
                                          <div key={m.label} className="space-y-2">
                                             <div className="flex justify-between text-[8px] font-black uppercase tracking-[0.3em] text-slate-500">
                                                <span>{m.label}</span>
                                                <span className="text-white">{m.val}g <span className="opacity-30">/ {m.target}g</span></span>
                                             </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                         {[
                           ...
                           <div key={meal.name} className={`p-6 bg-white/80 rounded-[2.5rem] border ${meal.bColor} shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center gap-6 group/meal`}>
                             <div className={`p-4 rounded-2xl ${meal.color} group-hover/meal:scale-110 transition-transform`}>
                               {meal.icon}
                             </div>
                             <div className="space-y-1">
                    ...
                    <div className="lg:col-span-12">
                      <div className="bg-slate-50/50 rounded-[2rem] border border-slate-100 flex-1 flex flex-col overflow-hidden">
                        <div className="p-6 border-b border-white flex justify-between items-center shrink-0">
                              ...
                              <AnimatePresence mode="popLayout">
                                {foodLog.map((f, idx) => (
                                  <motion.div 
                                    ...
                                    className="p-5 bg-white rounded-[2rem] border border-white shadow-[0_8px_30px_rgba(0,0,0,0.02)] group hover:shadow-[0_15px_40px_rgba(16,185,129,0.1)] hover:border-emerald-100/50 transition-all flex justify-between items-center relative overflow-hidden"
                                  >
                                     <div className="absolute left-0 top-0 w-1 h-full bg-emerald-500/0 group-hover:bg-emerald-500 transition-all"></div>
                                     <div className="flex items-center gap-5">
                                        <div className="w-14 h-14 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center text-2xl shadow-inner group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
                                     ...
                                     <div className="flex items-center gap-6">
                                        <div className="text-right">
                                           ...
                                           <div className="flex items-center gap-1.5 mt-2 justify-end">
                                              <span className="text-[8px] font-black text-blue-500 bg-blue-50 px-1.5 py-0.5 rounded-md">P:{f.protein}</span>
                                              <span className="text-[8px] font-black text-amber-500 bg-amber-50 px-1.5 py-0.5 rounded-md">C:{f.carbs}</span>
                                              <span className="text-[8px] font-black text-rose-500 bg-rose-50 px-1.5 py-0.5 rounded-md">F:{f.fats}</span>
                                           </div>
                                        </div>
                                        <button 
                                          onClick={() => deleteFoodEntry(f.id)}
                                          className="p-3 text-slate-200 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all opacity-0 group-hover:opacity-100 active:scale-90"
                                        >
                                          <Trash2 size={18} />
                                        </button>
                                     </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <motion.section 
                  ...
                  className="glass-card p-10 flex flex-col gap-10 relative overflow-hidden group"
                >
                   <div className="absolute top-0 right-0 p-12 opacity-[0.03] pointer-events-none group-hover:scale-110 transition-transform duration-1000">
                      <Droplet size={140} className="text-blue-600" />
                   </div>
                   <div className="flex justify-between items-center relative z-10">
                      <div className="flex items-center gap-4">
                         <div className="p-4 bg-blue-50 text-blue-600 rounded-2xl shadow-sm"><Droplet size={24} /></div>
                         <div>
                            <h3 className="font-black text-slate-900 text-lg leading-none">H2O Intake</h3>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Hydration Level</p>
                         </div>
                      </div>
                      <div className="text-right">
                         <span className="text-2xl font-black text-slate-900">{waterCount}</span>
                         <span className="text-xs font-bold text-slate-300 ml-1">/ {waterTarget} GL</span>
                      </div>
                   </div>
                   ...
                   <div className="space-y-4 relative z-10">
                      <div className="h-3 bg-slate-100 rounded-full overflow-hidden border-2 border-white shadow-inner">
                         <motion.div 
                           initial={{ width: 0 }}
                           animate={{ width: `${Math.min((waterCount / waterTarget) * 100, 100)}%` }}
                           className="h-full bg-blue-500 rounded-full shadow-[0_0_15px_rgba(59,130,246,0.3)] relative"
                         >
                      ...
                      <div className="flex items-center justify-between">
                         <div className="flex items-center gap-3">
                            <button onClick={() => setWaterCount(p => Math.max(0, p - 1))} className="w-12 h-12 bg-white text-slate-400 rounded-xl hover:bg-rose-50 hover:text-rose-500 border border-slate-100 transition-all font-black text-xl flex items-center justify-center shadow-sm">−</button>
                            <button onClick={() => setWaterCount(p => Math.min(20, p + 1))} className="w-12 h-12 bg-white text-slate-400 rounded-xl hover:bg-emerald-50 hover:text-emerald-500 border border-slate-100 transition-all font-black text-xl flex items-center justify-center shadow-sm">+</button>
                         </div>
                         <button onClick={() => saveTracker('water')} disabled={savingTracking === 'water'} className="px-6 py-4 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 disabled:opacity-50 flex items-center gap-2 font-black text-[10px] uppercase tracking-widest">
                            {savingTracking === 'water' ? <Zap className="animate-spin" size={14} /> : <Save size={14} />} 
                            Save Log
                         </button>
                      </div>
                   </div>
                ...
                >
                   <div className="absolute top-0 right-0 p-12 opacity-[0.03] pointer-events-none group-hover:scale-110 transition-transform duration-1000">
                      <Moon size={140} className="text-indigo-600" />
                   </div>
                   <div className="flex justify-between items-center relative z-10">
                      <div className="flex items-center gap-4">
                         <div className="p-4 bg-indigo-50 text-indigo-600 rounded-2xl shadow-sm"><Moon size={24} /></div>
                         <div>
                            <h3 className="font-black text-slate-900 text-lg leading-none">Sleep Depth</h3>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Recovery Quality</p>
                         </div>
                      </div>
                      <div className="text-right">
                         <span className="text-2xl font-black text-slate-900">{sleepHours.toFixed(1)}</span>
                         <span className="text-xs font-bold text-slate-300 ml-1">HR</span>
                      </div>
                   </div>
                   ...
                   <div className="space-y-4 relative z-10">
                      <div className="h-3 bg-slate-100 rounded-full overflow-hidden border-2 border-white shadow-inner">
                         <motion.div 
                           ...
                           className="h-full bg-indigo-500 rounded-full shadow-[0_0_15px_rgba(79,70,229,0.3)] relative"
                         >
                            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent animate-drift"></div>
                         </motion.div>
                      </div>
                      <div className="flex items-center justify-between">
                         <div className="flex items-center gap-3">
                            <button onClick={() => setSleepHours(p => Math.max(0, p - 0.5))} className="w-12 h-12 bg-white text-slate-400 rounded-xl hover:bg-rose-50 hover:text-rose-500 border border-slate-100 transition-all font-black text-xl flex items-center justify-center shadow-sm">−</button>
                            <button onClick={() => setSleepHours(p => Math.min(24, p + 0.5))} className="w-12 h-12 bg-white text-slate-400 rounded-xl hover:bg-indigo-50 hover:text-indigo-500 border border-slate-100 transition-all font-black text-xl flex items-center justify-center shadow-sm">+</button>
                         </div>
                         <button onClick={() => saveTracker('sleep')} disabled={savingTracking === 'sleep'} className="px-6 py-4 bg-indigo-600 text-white rounded-2xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 disabled:opacity-50 flex items-center gap-2 font-black text-[10px] uppercase tracking-widest">
                            {savingTracking === 'sleep' ? <Zap className="animate-spin" size={14} /> : <Save size={14} />} 
                            Save Log
                         </button>
                      </div>
                   </div>
